...
var champions = {
  ...
  function getchamp (champions){
    ...
    for (const key in champions.data) {
     ...
     filled += template.replace("{{title}}", value.title).replace( "{{name}}" ,value.name ).replace("{{id}}",value.id).replace("{{blurb}}",value.blurb).replace("{{tags}}",value.tags).replace("{{attack}}",value.info.attack).replace("{{defense}}",value.info.defense).replace("{{magic}}",value.info.magic).replace("{{difficulty}}",value.info.difficulty);
